import os
import numpy as np
import torch
from dual_alm_rnn_exp import DualALMRNNExp



# Initialize experiment
exp = DualALMRNNExp()

               
# Train RNN for each input asymmetry
for seed in range(50):

    for n in [0.1, 0.5, 1.0]:
        exp.configs['train_type'] = "train_type_modular_fixed_input_cross_hemi_cluster"
        exp.configs['init_cross_hemi_rel_factor'] = 0.5
        exp.configs['graded_signal_scale'] = 0.0
        exp.configs['graded_noise_scale'] = n
        exp.configs['xs_left_alm_amp'] = 1.0
        exp.configs['xs_right_alm_amp'] = 1.0
        exp.configs['random_seed'] = seed
        exp.configs['unfix_epoch'] = 0
        exp.configs['bs'] = 256
        exp.configs['n_neurons'] = 256
        exp.configs['lr'] = 1e-4
        exp.configs['n_epochs'] = 45
        exp.configs['num_cluster'] = [4,2]
        
        exp.train_type_modular_single_readout()